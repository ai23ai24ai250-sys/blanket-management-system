    ...
    <div class="space-y-8 animate-fadeIn">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800 shadow-sm">
        <div>
          ...
          <p class="text-sm text-slate-400">متابعة المبيعات الحية، المخزون، التحصيلات، ومستحقات الموردين</p>
        </div>
            ...
            <tbody>
              ...
              ` : recentOrders.map(o => `
                <tr>
                  ...
                  <td>
                    <span class="px-2.5 py-1 text-xs rounded-lg font-bold ${o.status === 'delivered' ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' : 'bg-slate-800 text-slate-300'}">
                      ${o.status === 'delivered' ? 'تم التوصيل' : o.status}
                    </span>
                  </td>
