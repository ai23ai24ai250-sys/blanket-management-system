/**
 * Google Sheets Two-Way Sync Module - Cloud Connected
 * ===================================================
 * Sync engine (transport-agnostic) + real Google Sheets API v4 transport.
 *
 * Architecture:
...
(function () {
  ...
  const SHEETS = [
    {
      ...
      protected: ['status', 'downPayment', 'refundedAmount', 'retainedDeposit', 'items', 'itemsSubtotal', 'totalAmount', 'remainingBalance', 'paidInFull', 'directShipping', 'shippingRevenueDeposit', 'supplierShipments', 'supplierDeficits'],
      headers: [
    ...
    },
    {
      ...
      idField: 'id',
      editable: ['name', 'phone', 'secondaryPhone', 'category', 'address', 'notes'],
  ...
  const TEXT_KEYS = new Set([
    'id', 'phone', 'secondaryPhone', 'code', 'customerPhone',
    'customerId', 'supplierId', 'entityId', 'productId'
  ]);
  ...
  // Reverse: "2026-08-01 15:13" (or "2026-08-01") -> ISO so imported data keeps
  // the same format the app already uses internally.
  ...
  // Google Sheets date serial number -> ISO (serial 1 = 1899-12-30).
  ...
  function normalizeRow(sheet, rawRow) {
    ...
    sheet.headers.forEach(h => {
      const v = rawRow[h.key] !== undefined ? rawRow[h.key] : rawRow[h.label];
  ...
  function applyCustomer(row, existing) {
    ...
    if (!existing) {
      ...
      if (window.findCustomerByPhone && window.findCustomerByPhone(phone)) throw new Error('رقم هاتف مسجل بالفعل لعميل آخر');
    ...
    for (const f of sheetOf('customers').editable) {
      ...
      if (f === 'phone') {
        ...
        if (window.findCustomerByPhone && window.findCustomerByPhone(phone) && window.findCustomerByPhone(phone).id !== id) {
          throw new Error('رقم هاتف مسجل بالفعل لعميل آخر');
        }
        payload.phone = phone;
      } else {
    ...
    if (Object.keys(payload).length) {
      ...
      payload.syncUpdatedAt = timeCell(row) || new Date().toISOString();
  ...
  function applySupplier(row, existing) {
    ...
    if (!existing) {
      ...
      if (window.findSupplierByPhone && phone && window.findSupplierByPhone(phone)) throw new Error('رقم هاتف مسجل بالفعل لمورد آخر');
      const doc = {
        id,
        name,
        phone,
  ...
  function applyOrder(row, existing) {
    ...
    if (payload.shippingCost !== undefined || payload.shippingPayer !== undefined ||
      ...
      const totalAmount = itemsSubtotal + (shipPayer === 'customer' ? shipCost : 0) + (exPayer === 'customer' ? exCost : 0);
      ...
      payload.remainingBalance = Math.max(0, totalAmount - downPayment);
  ...
  function applyPayment(row, existing) {
    ...
    if (!existing) {
      ...
      const doc = {
        ...
        amount,
        date: String(row.date || '').trim() || timeCell(row) || createdAt.split('T')[0],
  ...
  function applySupplierReturn(row, existing) {
    ...
    if (!existing) {
      ...
      const items = parseJSONField(row.items);
      const totalValue = row.totalValue === undefined || row.totalValue === null || row.totalValue === '' ? null : num(row.totalValue, 'قيمة المرتجع', false);
  ...
  NS.syncNow = async function (opts) {
    ...
    const result = { direction, startedAt: new Date().toISOString(), exported: null, imported: null };
    let totalRows = 0;
    _syncing = true;
    try {
      ...
      NS.saveConfig({ lastSyncAt: new Date().toISOString(), lastSyncDirection: direction, lastSyncRows: totalRows, lastSyncStatus: 'success', lastSyncError: '' });
      return result;
    } catch (e) {
      const msg = e && e.message ? e.message : String(e);
      recordLog({ direction, status: 'failed', rowsUpdated: totalRows, error: msg });
      NS.saveConfig({ lastSyncAt: new Date().toISOString(), lastSyncDirection: direction, lastSyncRows: totalRows, lastSyncStatus: 'failed', lastSyncError: msg });
      throw e;
    } finally {
      _syncing = false;
    }
  ...
  function recordLog(entry) {
    ...
    log.unshift(Object.assign({ timestamp: new Date().toISOString() }, entry));
    writeJSON(LOG_KEY, log.slice(0, 200));
  }
  function recordAudit(entry) {
    const recs = NS.getSyncAuditRecords();
    recs.unshift(Object.assign({ timestamp: new Date().toISOString() }, entry));
        ...
        return ensureTabs().catch(function () {}).then(function () {
          ...
          for (let i = 1; i < values.length; i++) {
            ...
            headers.forEach((h, c) => {
              const v = values[i][c];
              if (v !== undefined && v !== '') {
                rec[h] = (dateCols[c] && typeof v === 'number') ? serialToIso(v) : v;
              }
            });
  ...
  NS.renderSyncPanel = function (el, opts) {
    ...
    wrap.dir = 'rtl';
    const inputStyle = 'width:100%;padding:6px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:#e2e8f0';
    const btnStyle = 'padding:8px 14px;border-radius:8px;border:none;color:#fff;cursor:pointer;font-weight:bold';
    wrap.innerHTML = [
      '<div style="display:flex;flex-direction:column;gap:10px;font-size:13px">',
      '  <label>Spreadsheet ID' +
      '    <input id="gs-sheet-id" type="text" placeholder="مثال: 1986eZQKcjK..." style="' + inputStyle + '" value="' + (cfg.spreadsheetId || '') + '">' +
      '  </label>',
      ...
      '  </div>',
      '  <div id="gs-status" style="color:#e2e8f0;font-size:12px;background:#0f172a;border:1px solid #334155;border-radius:8px;padding:8px"></div>',
      '  <div id="gs-last" style="color:#94a3b8;font-size:12px">آخر مزامنة: ' + (cfg.lastSyncStatus || 'لا توجد') + (cfg.lastSyncAt ? ' — ' + cfg.lastSyncAt : '') + (cfg.lastSyncRows ? ' (' + cfg.lastSyncRows + ' صف)' : '') + '</div>',
      '  <div style="font-size:11px;color:#64748b">سجل آخر 10 عمليات مزامنة:</div>',
      '  <div id="gs-audit" style="color:#94a3b8;font-size:11px;max-height:160px;overflow:auto"></div>',
      '</div>'
    ].join('');
    el.innerHTML = '';
    el.appendChild(wrap);
    ...
    const refreshExpiredToken = () => {
      ...
      if (window.location && window.location.protocol === 'file:') {
        return Promise.reject(new Error('انتهت صلاحية التوكن المحفوظ — ألصق توكن Access جديد (تسجيل الدخول عبر Google يحتاج تشغيل النظام عبر https)'));
      }
        ...
        const client = window.google.accounts.oauth2.initTokenClient({
          ...
          callback: (resp) => {
            if (resp && resp.access_token) {
              NS.saveConfig({ accessToken: resp.access_token, tokenExpiresAt: Date.now() + (Number(resp.expires_in) || 3600) * 1000 });
              renderTokenMeta();
              resolve();
            } else {
    ...
    wrap.querySelector('#gs-connect').addEventListener('click', () => {
          ...
          _tokenClient = window.google.accounts.oauth2.initTokenClient({
            ...
            callback: (resp) => {
              if (resp && resp.access_token) {
                ...
                NS.saveConfig({ accessToken: resp.access_token, tokenExpiresAt: exp });
                try {
                  NS.connectGoogleSheets();
      ...
      NS.saveConfig({ accessToken: token, tokenExpiresAt: 0 });
      try {
        NS.connectGoogleSheets();
