 ...
 * Enforces Strict Dynamic Role-Based Access Control (RBAC)
 */
...
class BMSApp {
  ...
  checkAuth() {
    if (!window.isAuthenticated()) {
      ...
      // Storekeeper default view is Products
  ...
  updateUserUI() {
    ...
    // 🔒 Dynamic Navigation Visibility Rules by Role
    document.querySelectorAll('[data-nav]').forEach(el => {
      const targetNav = el.getAttribute('data-nav');
      if (!targetNav) return;

      if (role === 'storekeeper') {
        // Storekeeper can ONLY see Products & Inventory
        ...
        // Sales Employee cannot see User Management or Reports
        if (targetNav === 'users' || targetNav === 'reports' || targetNav === 'settings') {
          el.classList.add('hidden');
        } else {
          el.classList.remove('hidden');
        }
    ...
    // Hide or Show Quick New Order button for storekeeper
  ...
  navigateTo(viewName) {
    ...
    if (role === 'storekeeper' && viewName !== 'products') {
      ...
      viewName = 'products';
    } else if (role === 'employee' && (viewName === 'users' || viewName === 'reports')) {
      window.showToast('عفواً! ليس لديك صلاحية الوصول لهذه الصفحة المالية/الإدارية', 'error');
      viewName = 'dashboard';
    }
