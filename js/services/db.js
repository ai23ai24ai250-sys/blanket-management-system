...
//   - Every db.js write choke point (add/update/delete/saveCollection) mutates
//     ONLY the in-RAM firestoreCache — never localStorage, never Firestore,
//     never the bms_pending_ops queue, never tombstones.
//   - Realtime Firestore listeners are detached (frozen connection): no server
//     snapshot can leak into the sandbox, and no sandbox change reaches a server.
...
// state naturally — the flag is never persisted, so a reload always boots back
...
function _sandboxDeepClone(value) {
  ...
  catch (e) { return value == null ? [] : value; }
}
...
window.setSandboxMode = function(on) {
  return on ? window.enterSandboxMode() : window.exitSandboxMode();
};

window.isSandboxActive = function() { return !!window.isSandboxMode; };
