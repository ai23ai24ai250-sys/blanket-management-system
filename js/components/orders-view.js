    ...
    <div class="space-y-6 animate-fadeIn">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800">
        ...
        <div class="flex items-center gap-3">
          ...
          <!-- V3.26 — Order status filter dropdown (combined with the search box) -->
          <select id="orders-status-filter" title="تصفية حسب حالة الطلب" class="w-44 px-3 py-2.5 bg-slate-800/80 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:border-brand-500 transition-all cursor-pointer">
            <option value="">جميع الحالات</option>
            <option value="new">قيد الانتظار (new)</option>
            <option value="delivered">تم التوصيل (delivered)</option>
            <option value="completed">مكتمل ومسدد (completed)</option>
  ...
  return ordersList.map(o => {
    ...
    return `
      <tr>
        ...
        <td class="num-font font-bold ${window.getOrderRemainingAmount(o) > 0 ? 'text-rose-400' : 'text-slate-400'}">${window.formatCurrency(window.getOrderRemainingAmount(o))}</td>
...
window.setupOrdersEvents = function(container, refreshFn) {
  ...
  const statusFilter = container.querySelector('#orders-status-filter');
  const tableBody = container.querySelector('#orders-table-body');

  // V3.26 — one shared filter pipeline: free-text search AND status dropdown
  // combine, so picking a status instantly narrows the visible table rows.
  const applyFilters = () => {
    const q = searchInput ? searchInput.value : '';
    const status = statusFilter ? statusFilter.value : '';
    let filtered = window.searchOrders(q);
    if (status) {
      filtered = filtered.filter(o => String(o.status) === status);
    }
  ...
  const contentHTML = `
    <div class="space-y-4">
      ...
      <div class="p-4 bg-slate-950/80 rounded-xl border border-slate-800 space-y-1.5 text-xs">
        ...
        ` : ''}
        <div class="flex justify-between text-slate-200 font-bold border-t border-slate-800 pt-1"><span>المتبقي الآجل:</span><span class="font-extrabold text-rose-400 num-font text-sm">${window.formatCurrency(window.getOrderRemainingAmount(order))}</span></div>
...
window.openOrderStatusModal = function(orderId, currentStatus, refreshParentFn) {
  ...
  // V3.15.2 — Build the allowed next-status options from the state machine so the
  // UI can never offer an invalid transition (e.g. delivered → cancelled).
  const statusLabels = {
  ...
  const allowedTargets = (window.ORDER_STATUS_TRANSITIONS && window.ORDER_STATUS_TRANSITIONS[sourceStatus]) || [];
  const statusOptionsHTML = allowedTargets.map((s) => {
    const label = (s === 'new' && (sourceStatus === 'cancelled' || sourceStatus === 'returned'))
      ? 'جديد (إعادة تفعيل)'
      : (statusLabels[s] || s);
    return `<option value="${s}">${label}</option>`;
  }).join('');

  // V3.15.3 — Re-activation (cancelled/returned → new): the admin may record a NEW
  // deposit actually collected from the customer when re-arming the order. Defaults
  // to the retained portion (downPayment − already refunded) for backward-compat.
  const canReactivate = sourceStatus === 'cancelled' || sourceStatus === 'returned';
  const retainedDeposit = Math.max(0, deposit - (Number(order && order.refundedAmount) || 0));
    ...
    <form id="form-update-order-status" class="space-y-4">
      ...
      <div>
        ...
        <select id="select-new-order-status" class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-white font-bold text-xs">
          ${statusOptionsHTML || '<option value="">— لا توجد انتقالات مسموحة —</option>'}
        </select>
      </div>
      ...
      ${canReactivate ? `
      <div id="reactivation-section" class="hidden p-3 bg-sky-950/30 rounded-xl border border-sky-800/40 space-y-3">
        <label class="block text-xs font-bold text-sky-300 mb-1.5">مبلغ العربون المستلم عند إعادة التفعيل</label>
        <input type="number" id="reactivation-amount" min="0" step="any" value="${retainedDeposit}" class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-sky-400 font-extrabold num-font">
        <p class="text-[11px] text-sky-400/80">الافتراضي: المحتفظ به من العربون بعد أي استرداد سابق (${window.formatCurrency(retainedDeposit)}). يُسجَّل المبلغ المكتوب هنا كاستلام فعلي في خزينة العميل عند إعادة التفعيل.</p>
      </div>
      ` : ''}
  ...
  window.openModal({
    ...
    onRender: (modalEl, closeModal) => {
      ...
      const reactivationSection = modalEl.querySelector('#reactivation-section');
      const reactivationInput = modalEl.querySelector('#reactivation-amount');
      ...
      const refreshRefundVisibility = () => {
        ...
        // V3.15.3 — Re-activation deposit field appears only when moving back to "جديد".
        if (reactivationSection) {
          reactivationSection.classList.toggle('hidden', statusSelect.value !== 'new');
        }
      };
      ...
      modalEl.querySelector('#form-update-order-status').onsubmit = (e) => {
        ...
        try {
          // V3.15.3 — Re-activation deposit: pass the entered value (if any) so the
          // re-credit records the ACTUAL amount received instead of forcing the
          // retained default (downPayment − refundedAmount).
          let reactivationDeposit = 0;
          if (newStatus === 'new' && reactivationInput && reactivationInput.value !== '') {
            reactivationDeposit = parseFloat(reactivationInput.value) || 0;
          }
          window.updateOrderStatus(orderId, newStatus, refundAmount, reactivationDeposit);
