...
(function () {
  ...
  function _persistConfig(cfg) {
    if (!window.db || !window._authUser) return;
    try {
      window.db.collection('settings').doc('syncConfig').set({
    ...
    } catch (e) { /* never break the config write path */ }
  }
  ...
  NS.hydrateConfigFromCloud = async function () {
    if (!window.db || !window.auth || !window._authUser) return;
    try {
    ...
    } catch (e) { /* silent: rules may deny before deploy; fall back to local */ }
  };
  ...
  NS.syncWithGoogleSheets = async function () {
    ...
    if (cfg.accessToken && exp > 0 && Date.now() > exp && cfg.refreshToken) {
  ...
  NS.renderSyncPanel = function (el, opts) {
    ...
    wrap.innerHTML = [
      ...
      '    <input id="gs-sheet-id" type="text" placeholder="مثال: 1986eZQKcjK..." style="' + inputStyle + '" value="' + (cfg.spreadsheetId || '') + '" disabled>' +
      '  </label>',
      '  <label>Client ID (لتسجيل الدخول عبر حساب Google — اختياري)' +
      '    <input id="gs-client-id" type="text" placeholder="xxxx.apps.googleusercontent.com" style="' + inputStyle + '" value="' + (cfg.clientId || '') + '" disabled>' +
      '  </label>',
      '  <label>Client Secret (مطلوب مع Refresh Token للتجديد التلقائي — اختياري)' +
      '    <input id="gs-client-secret" type="password" placeholder="GOCSPX-..." style="' + inputStyle + '" value="' + (cfg.clientSecret || '') + '" disabled>' +
      '  </label>',
      ...
      '    <span id="gs-lock-note" style="color:#94a3b8;font-size:11px">الحقول الحساسة مقفلة — أدخل كلمة سر المدير لتعديلها</span>' +
      '  </div>',
    ...
    const requestUnlock = () => {
      ...
      window.openModal({
        ...
        maxWidth: 'max-w-md',
        contentHTML:
          '<p style="color:#94a3b8;font-size:13px">أدخل كلمة سر المدير لتعديل Spreadsheet ID و Client ID و Client Secret.</p>' +
          '<label style="display:block;margin-top:12px;font-size:13px">كلمة سر المدير' +
          '  <input id="unlock-admin-pass" type="password" placeholder="••••••••" style="width:100%;padding:8px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:#e2e8f0;margin-top:4px">' +
          '</label>' +
