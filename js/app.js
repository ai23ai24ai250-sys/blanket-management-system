...
class BMSApp {
  ...
  updateUserUI() {
    ...
    if (user) {
      ...
      if (this.userRoleBadge) {
        this.userRoleBadge.textContent = user.role === 'admin' ? 'مدير' : user.role === 'storekeeper' ? 'مخزن' : 'موظف';
      }

      // Hide or Show Admin-Only Navigation Buttons
      ...
      const usersNavBtn = document.getElementById('nav-btn-users');
      if (usersNavBtn) {
        if (isAdminUser) usersNavBtn.classList.remove('hidden');
        else usersNavBtn.classList.add('hidden');
      }
    }
  }
  ...
  navigateTo(viewName) {
    ...
    // RBAC Guard for User Management
    if (viewName === 'users' && !window.isAdmin()) {
      window.showToast('عفواً! صفحة الموظفين مقتصرة على مدير النظام فقط', 'error');
      viewName = 'dashboard';
    }
