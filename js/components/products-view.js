 ...
 * Supports Shipment Quantity Addition (توريد شحنة) & Product Edits
 */
      ...
      <tr class="${isNegative ? 'bg-rose-950/20' : isLowStock ? 'low-stock-row' : ''}">
        ...
        <td>
          <div class="flex items-center gap-2">
            ...
            <!-- Delete Button (Admin / Authorized) -->
...
function openShipmentModal(product, refreshParentFn) {
  const contentHTML = `
    <form id="form-add-shipment" class="space-y-4">
      ...
      <div>
        <label class="block text-xs font-bold text-slate-300 mb-1.5">الكمية المضافة للأنشطة / الشحنة الجديدة * (قطعة)</label>
        <input type="number" id="shipment-qty" min="1" required placeholder="مثال: 50" class="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-emerald-400 font-extrabold text-xl num-font text-center">
      </div>

      <div>
        <label class="block text-xs font-bold text-slate-300 mb-1.5">بيانات وملاحظات الشحنة / المورد (اختياري)</label>
      ...
      <div class="flex justify-end gap-3 pt-2">
        <button type="submit" class="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm rounded-xl shadow-lg shadow-emerald-600/20 flex items-center gap-2">
          ...
          <span>إضافة الكمية إلى المخزون</span>
        </button>
      </div>
    </form>
  `;
  ...
  window.openModal({
    title: `📦 توريد شحنة كمية جديدة: ${product.name}`,
    icon: 'package-plus',
    contentHTML,
    onRender: (modalEl, closeModal) => {
      modalEl.querySelector('#form-add-shipment').onsubmit = (e) => {
        e.preventDefault();
        const qty = modalEl.querySelector('#shipment-qty').value;
        const notes = modalEl.querySelector('#shipment-notes').value;

        try {
          const newStock = window.addStockShipment(product.id, qty, notes);
          window.showToast(`تمت إضافة ${qty} قطعة بنجاح. المخزون الجديد: ${newStock} قطعة`, 'success');
          closeModal();
