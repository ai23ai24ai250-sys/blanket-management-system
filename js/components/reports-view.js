      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800">
        <div>
          <h1 class="text-2xl font-bold text-white mb-1 flex items-center gap-2">
            ...
            <span>التقارير اليومية وكشوفات الحسابات</span>
          </h1>
          <p class="text-sm text-slate-400">تقارير المبيعات، كشف حساب العميل، كشف حساب المورد، وتصدير إكسل بنقرة واحدة</p>
        </div>
        ...
        <div class="flex items-center gap-3">
          <button id="btn-export-excel" class="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow-md transition-all flex items-center gap-2">
            ...
            <span>تصدير التقرير الحالي إلى Excel</span>
          </button>
        </div>
      </div>
...
function renderSalesReport(orders) {
  ...
  const totalRemaining = orders.reduce((sum, o) => sum + (Number(o.remainingBalance) || 0), 0);

  return `
    <div class="space-y-4">
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="p-4 bg-slate-850 rounded-xl border border-slate-800">
          <span class="text-xs text-slate-400 font-bold block mb-1">إجمالي قيمة الفواتير</span>
          <span class="text-xl font-extrabold text-white num-font">${window.formatCurrency(totalAmount)}</span>
        </div>
        <div class="p-4 bg-slate-850 rounded-xl border border-slate-800">
          <span class="text-xs text-slate-400 font-bold block mb-1">إجمالي المبالغ المحصلة</span>
          <span class="text-xl font-extrabold text-emerald-400 num-font">${window.formatCurrency(totalDownPayments)}</span>
        </div>
        <div class="p-4 bg-slate-850 rounded-xl border border-slate-800">
          <span class="text-xs text-slate-400 font-bold block mb-1">إجمالي المبالغ الآجلة (المتبقية)</span>
          <span class="text-xl font-extrabold text-rose-400 num-font">${window.formatCurrency(totalRemaining)}</span>
        </div>
      </div>
      ...
      <div class="overflow-x-auto rounded-xl border border-slate-800">
        <table class="data-table">
          ...
          <tbody>
            ${orders.length === 0 ? `
              <tr><td colspan="8" class="text-center py-6 text-slate-500">لا توجد مبيعات مسجلة</td></tr>
            ` : orders.map(o => `
              <tr>
...
window.setupReportsEvents = function(container) {
  ...
  tabCustomer.onclick = () => {
    ...
    const updateCustStatement = () => {
      ...
      custDetails.innerHTML = `
        ...
        <table class="data-table">
          <thead>
            <tr><th>رقم الطلب</th><th>المبلغ الإجمالي</th><th>المسدد</th><th>المتبقي</th><th>التاريخ</th></tr>
          </thead>
          <tbody>
            ${cOrders.map(o => `
              <tr>
  ...
  tabSupplier.onclick = () => {
    ...
    const updateSupStatement = () => {
      ...
      if (!supplier) return;
      const sPayments = window.getPayments().filter(p => p.entityId === sId);
      ...
      supDetails.innerHTML = `
        ...
        <table class="data-table">
          <thead>
            <tr><th>رقم الإيصال</th><th>المبلغ المسدد</th><th>طريقة الدفع</th><th>التاريخ</th></tr>
          </thead>
          <tbody>
            ${sPayments.map(p => `
              <tr>
