 ...
 * Requires Supplier Binding, SKU Code Search & Display, Supplier Shipment Debt Accumulation
 */
...
function openProductModal(productToEdit = null, refreshParentFn = null) {
  ...
  if (!isEdit && suppliers.length === 0) {
    window.showToast('يرجى إضافة مورد واحد على الأقل في دليل الموردين أولاً قبل إضافة منتجات جديدة', 'warning');
  }
  ...
  const contentHTML = `
    <form id="form-product" class="space-y-4">
      ...
      <div>
        ...
        <select id="prd-supplier" required class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-white font-bold">
          <option value="">-- اختر المورد المصنع --</option>
          ${suppliers.map(s => `
            <option value="${s.id}" data-name="${s.name}" ${isEdit && productToEdit.supplierId === s.id ? 'selected' : ''}>
