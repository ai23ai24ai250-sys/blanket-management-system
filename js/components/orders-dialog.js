 ...
 * Live Stock & Deficit Indicator, Strict Down Payment Validation & Multi-click Protection
 */
  ...
  window.openModal({
    ...
    onRender: (modalEl, closeModal) => {
      ...
      const updateCalculations = () => {
        ...
        modalEl.querySelectorAll('.product-item-row').forEach(row => {
          ...
          if (pObj && indicator) {
            ...
            if (qty > currentStock) {
              ...
              indicator.innerHTML = `<span class="text-rose-400 font-bold">⚠️ تنبيه: عجز بقيمة ${deficit} قطعة وسيصبح المخزون بالسالب (${currentStock - qty})</span>`;
            } else {
        ...
        // 🔒 Exact Comparison: Compare Down Payment (rawDp) vs Total Invoice Amount (total)
        const rawDp = parseFloat(downPaymentInput.value) || 0;
        
        if (rawDp > total && total > 0) {
          dpValidationMsg.textContent = `⚠️ تنبيه: الدفعة المقدمة (${window.formatCurrency(rawDp)}) لا يمكن أن تتجاوز إجمالي الفاتورة (${window.formatCurrency(total)})`;
          dpValidationMsg.classList.remove('hidden');
        } else {
          dpValidationMsg.classList.add('hidden');
        }

        const dp = Math.min(total, rawDp);
      ...
      modalEl.querySelector('#form-new-order').onsubmit = (e) => {
        ...
        if (!phoneValidation.isValid) {
          ...
          return false; // BLOCK SUBMIT COMPLETELY!
        }
        ...
        // 🔒 STRICT VALIDATION: Down Payment vs Total Invoice Amount
        const totalInvoiceAmount = validItems.reduce((sum, item) => sum + (Number(item.quantity) * Number(item.sellingPrice)), 0);
