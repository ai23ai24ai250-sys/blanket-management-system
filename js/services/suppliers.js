...
// numbers. Exported as shared helpers so EVERY entry point that adds or edits
// a supplier (main screen modal, quick-add popups, Google Sheets sync, ...)
// enforces the exact same rule AND the exact same user-facing rejection
// message including the conflicting supplier name.
window.findSupplierPhoneConflict = function(phone, secondaryPhone, excludeId = '') {
  return (phone ? window.findSupplierByPhone(phone, excludeId) : null)
...
window.assertSupplierPhoneAvailable = function(phone, secondaryPhone, excludeId = '') {
  const conflict = window.findSupplierPhoneConflict(phone, secondaryPhone, excludeId);
  if (conflict) {
...
window.createSupplier = function(data) {
  ...
  window.assertSupplierPhoneAvailable(phone, secondaryPhone, '');
...
window.updateSupplier = function(id, updatedFields) {
  ...
  window.assertSupplierPhoneAvailable(phone, secondaryPhone, id);
  if (secondaryPhone && secondaryPhone === phone) {
    throw new Error('رقم الهاتف الثانوي لا يمكن أن يطابق الرقم الرئيسي');
  }
