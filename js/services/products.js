...
// Allows Negative Inventory so merchants can track exact stock deficit
...
// Add New Stock Shipment / Inventory Supply (إضافة شحنة / توريد كمية جديدة)
window.addStockShipment = function(id, addedQuantity, shipmentNotes = '') {
  const product = window.getProductById(id);
  if (!product) {
    throw new Error('المنتج غير موجود');
  }

  const addedQty = Number(addedQuantity);
  if (isNaN(addedQty) || addedQty <= 0) {
    throw new Error('يرجى إدخال كمية شحنة صحيحة أكبر من الصفر');
  }

  const updatedStock = Number(product.stock) + addedQty;
  const noteLog = shipmentNotes ? ` [توريد شحنة: +${addedQty} (${shipmentNotes})]` : '';
  const currentNotes = product.notes || '';

  window.updateProduct(id, {
    stock: updatedStock,
    notes: (currentNotes + noteLog).trim()
  });
