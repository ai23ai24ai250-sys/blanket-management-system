...
window.getCurrentUser = function() {
  if (window.auth && window.auth.currentUser) {
    ...
    let sessionPwd = '';
    if (session) {
      try { sessionPwd = JSON.parse(session).password || ''; } catch(e) {}
    }
    ...
    return {
      ...
      password: matched ? (matched.password || sessionPwd) : sessionPwd
    };
  }
...
window.verifyAdminPassword = function(enteredPassword) {
  ...
  // 1. Check against password stored in Firestore users document
  if (activeUserDoc && activeUserDoc.password && activeUserDoc.password.trim()) {
    return activeUserDoc.password.trim() === cleanInput;
  }

  // 2. Check against active session password
  if (currentUser.password && currentUser.password.trim()) {
    return currentUser.password.trim() === cleanInput;
  }

  // 3. Fallback check for initial admin account if password not explicitly set
  return currentUser.role === 'admin' && cleanInput.length >= 3;
};
