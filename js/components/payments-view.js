    ...
    <form id="form-record-payment" class="space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="date" id="pay-date" value="${getCairoFormattedDate().slice(0, 10)}" class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-white num-font">
        </div>
      </div>
  ...
  window.openModal({
    ...
    onRender: (modalEl, closeModal) => {
      ...
      const checkAmountValidation = () => {
        ...
        if (amt > max) {
          // V3.20 — Overpay protection: block with the exact Arabic message and
          // AUTO-FILL the input with the exact remaining balance so the admin
          // reviews the corrected amount before confirming. The payment is then
          // recorded immediately as an approved ledger row, so the stored
          // balance ties to the payments ledger 100%.
          const overMsg = typeSelect.value === 'supplier'
            ? 'المبلغ المدخل أكبر من إجمالي المديونية المستحقة للمورد'
            : 'المبلغ المدخل أكبر من إجمالي المديونية المتبقية على العميل';
          validationMsg.textContent = `⚠️ ${overMsg} (${window.formatCurrency(max)})` + (max > 0 ? ` — تم تعبئة الخانة تلقائياً بالمبلغ المتبقي (${window.formatCurrency(max)}) للمراجعة ثم التأكيد` : ' — لا يجوز التحصيل من حساب خالٍ من المديونية');
          validationMsg.classList.remove('hidden');
          if (max > 0 && amountInput.value !== String(max)) {
      ...
      modalEl.querySelector('#form-record-payment').onsubmit = (e) => {
        ...
        if (amount > maxRemaining) {
          const overMsg = type === 'supplier'
            ? 'المبلغ المدخل أكبر من إجمالي المديونية المستحقة للمورد'
            : 'المبلغ المدخل أكبر من إجمالي المديونية المتبقية على العميل';
          window.showToast(`${overMsg} (${window.formatCurrency(maxRemaining)}) — تم منع الحفظ وإيقاف العملية`, 'error');
