...
window.openNewOrderModal = function(onSuccessCallback = null) {
  ...
  const contentHTML = `
    <form id="form-new-order" class="space-y-6">
      ...
      <!-- F4 — Cashier / quick-sale mode (بيع فوري من المعرض) -->
      <div class="bg-slate-950/40 p-4 rounded-xl border border-amber-700/40 space-y-3">
        <label class="flex items-center justify-between gap-3 cursor-pointer select-none">
          <span class="text-sm font-bold text-amber-300 flex items-center gap-2">
            ...
            <span>وضع البيع الفوري (كاشير المعرض) 🏬</span>
          </span>
          <span class="relative inline-flex items-center shrink-0">
            <input type="checkbox" id="order-cashier-mode" class="peer sr-only">
            <span class="w-12 h-6 bg-slate-700 rounded-full peer-checked:bg-amber-600 transition-all"></span>
            <span class="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-all peer-checked:translate-x-6"></span>
          </span>
        </label>
        <p id="cashier-mode-hint" class="text-[11px] font-bold text-amber-400/90 hidden">
          بيع فوري نقداً بالكامل: يُسجَّل الطلب «مكتمل» بدفع كامل الفاتورة باسم «عميل معرض» (أو الاسم الذي تُدخله)، بدون شحن/مصروفات إضافية/نوع عربون، ورقم الهاتف اختياري.
        </p>
      </div>
        ...
        <div id="registered-address-wrap" class="space-y-3 hidden">
          ...
          <div id="order-new-address-form" class="hidden grid grid-cols-1 sm:grid-cols-2 gap-3 p-3 bg-slate-950/40 rounded-xl border border-slate-800">
            ...
            <div>
              ...
              <input type="text" id="order-new-addr-city-manual" placeholder="اكتب اسم المدينة / المركز يدوياً..." class="hidden mt-2 w-full px-4 py-2.5 bg-slate-900 border border-amber-700/60 rounded-xl text-white font-bold transition-all">
            </div>
            <div class="sm:col-span-2">
        ...
        <div id="manual-address-wrap" class="space-y-3">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" id="order-cust-city-manual" placeholder="اكتب اسم المدينة / المركز يدوياً..." class="hidden mt-2 w-full px-4 py-2.5 bg-slate-900 border border-amber-700/60 rounded-xl text-white font-bold transition-all">
            </div>
          </div>
      ...
      <div id="shipping-extra-section" class="bg-slate-950/40 p-4 rounded-xl border border-slate-800 space-y-4">
      ...
      <div id="deposit-type-section" class="bg-slate-950/40 p-4 rounded-xl border border-slate-800 space-y-2">
        <div class="flex items-center justify-between">
      ...
      <div id="payment-status-section" class="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-950/40 p-4 rounded-xl border border-slate-800">
        <div>
  ...
  window.openModal({
    ...
    onRender: (modalEl, closeModal) => {
      ...
      const cityManualInput = modalEl.querySelector('#order-cust-city-manual');
      ...
      const newAddrCityManualInput = modalEl.querySelector('#order-new-addr-city-manual');
      ...
      const cashierToggle = modalEl.querySelector('#order-cashier-mode');
      const cashierHint = modalEl.querySelector('#cashier-mode-hint');
      const shippingExtraSection = modalEl.querySelector('#shipping-extra-section');
      const depositTypeSection = modalEl.querySelector('#deposit-type-section');
      const paymentStatusSection = modalEl.querySelector('#payment-status-section');
      ...
      // F1 — Dynamic governorate→city picker (built-in cities + saved manual cities)
      ...
      const populateNewAddrCities = () => {
        ...
        newAddrCitySelect.innerHTML = window.citySelectOptions(selectedGov, '');
        if (newAddrCityManualInput) { newAddrCityManualInput.value = ''; newAddrCityManualInput.style.display = 'none'; }
      };
      window.setupCitySelect({
        governorateSelect: newAddrGovSelect,
        citySelect: newAddrCitySelect,
        manualInput: newAddrCityManualInput
      });
      ...
      btnSaveNewAddress.onclick = () => {
        ...
        const city = window.getEffectiveCity(newAddrCitySelect, newAddrCityManualInput);
      ...
      // F4 — Cashier (quick-sale) mode toggle
      const applyCashierMode = (on) => {
        if (cashierHint) cashierHint.classList.toggle('hidden', !on);
        if (shippingExtraSection) shippingExtraSection.classList.toggle('hidden', on);
        if (depositTypeSection) depositTypeSection.classList.toggle('hidden', on);
        if (paymentStatusSection) paymentStatusSection.classList.toggle('hidden', on);
        phoneInput.required = !on;
        if (on) {
          nameInput.required = false;
          phoneInput.placeholder = 'اختياري — يُسجَّل الطلب باسم عميل المعرض';
          if (statusSelect) statusSelect.value = 'completed';
          if (downPaymentInput) downPaymentInput.value = '0';
          if (shippingCostInput) shippingCostInput.value = '0';
          if (extraExpensesInput) extraExpensesInput.value = '0';
          if (depositTypeSelect) depositTypeSelect.value = 'custom';
        } else {
      ...
      if (cashierToggle) {
        cashierToggle.addEventListener('change', () => applyCashierMode(cashierToggle.checked));
      }
      ...
      modalEl.querySelector('#form-new-order').onsubmit = (e) => {
        ...
        const cashierMode = cashierToggle ? cashierToggle.checked : false;

        // 🔒 STRICT VALIDATION: Phone Number (skipped in cashier mode — optional)
        ...
        // "مكتمل نهائي" (أو بيع فوري كاشير) يُسدد إجمالي الفاتورة بالكامل (حماية إضافية على مستوى الواجهة)
        const orderStatus = cashierMode ? 'completed' : statusSelect.value;
        ...
        // F4 — Cashier mode: optional customer defaults to "عميل معرض"
        const customerName = cashierMode && !nameInput.value.trim() ? 'عميل معرض' : nameInput.value;
