...
class BMSApp {
  constructor() {
    ...
    // V3.25: Sandbox Mode (وضع الاختبار) UI
    this.sandboxToggleBtn = document.getElementById('btn-sandbox-toggle');
    this.sandboxToggleLabel = document.getElementById('sandbox-toggle-label');
    this.sandboxBanner = document.getElementById('sandbox-banner');
  ...
  init() {
    ...
    // 4. Sandbox Mode UI (وضع الاختبار)
    this.setupSandboxUI();
  }

  setupSandboxUI() {
    const banner = this.sandboxBanner;
    const toggleBtn = this.sandboxToggleBtn;
    const toggleLabel = this.sandboxToggleLabel;

    const renderSandboxState = () => {
      const active = window.isSandboxActive && window.isSandboxActive();
      if (banner) banner.classList.toggle('hidden', !active);
      if (toggleBtn) {
        toggleBtn.classList.toggle('bg-amber-600', active);
        toggleBtn.classList.toggle('border-amber-500', active);
        toggleBtn.classList.toggle('bg-slate-800/80', !active);
        toggleBtn.classList.toggle('border-slate-700', !active);
      }
      if (toggleLabel) toggleLabel.textContent = active ? 'إنهاء الاختبار' : 'وضع الاختبار';
      if (active && window.showToast) {
        window.showToast('🧪 وضع الاختبار نشط — كل التغييرات داخل الذاكرة فقط', 'info');
      }
    };
    ...
    if (toggleBtn) {
      toggleBtn.addEventListener('click', () => {
        const active = window.isSandboxActive && window.isSandboxActive();
        const ok = active ? window.exitSandboxMode() : window.enterSandboxMode();
        if (ok !== false) renderSandboxState();
      });
    }

    window.addEventListener('bms-sandbox-changed', () => renderSandboxState());
    renderSandboxState();
  }
  ...
  checkAuth() {
    if (!window.isAuthenticated()) {
      ...
      // V3.17 — pull the cloud copy of general settings (app name/theme/color)
      // so another device's changes apply on this browser right after login.
      if (window.hydrateGeneralSettings) window.hydrateGeneralSettings();
    }
  }
  ...
  updateUserUI() {
    ...
    document.querySelectorAll('[data-nav]').forEach(el => {
      ...
      if (role === 'storekeeper') {
        ...
        if (targetNav === 'users' || targetNav === 'reports' || targetNav === 'suppliers' || targetNav === 'payments') {
          el.style.display = 'none';
          el.classList.add('hidden');
        } else {
          el.style.display = '';
    ...
    // V3.18 — admin-only menu items (e.g. "إعدادات الربط والسحابة 🔐") hidden for non-admins
  ...
  registerGlobalEvents() {
    ...
    // V3.21 — Show/Hide password toggle on the login screen. Icons sit on the
    // RIGHT side (far from the left-aligned text) so typing is never covered.
    ...
    if (pwToggleBtn) {
      ...
      pwToggleBtn.onclick = () => {
        ...
        const eye = pwToggleBtn.querySelector('.toggle-eye');
        const eyeOff = pwToggleBtn.querySelector('.toggle-eye-off');
        if (eye) eye.classList.toggle('hidden', show);
        if (eyeOff) eyeOff.classList.toggle('hidden', !show);
      };
    }

    // Self-service Password Change (strict 3-field flow; no "current password"
    // required when the account has no password registered yet — e.g. the seed
    // admin right after a wipe).
    if (this.changePasswordBtn) {
      this.changePasswordBtn.onclick = () => {
        const noPassSet = !(window.adminPasswordConfigured ? window.adminPasswordConfigured() : true);
        window.openModal({
          ...
          contentHTML: `
            <form id="form-change-password" class="space-y-4">
              ${noPassSet
                ? '<div style="background:rgba(245,158,11,0.12);border:1px solid rgba(245,158,11,0.4);color:#fbbf24;padding:10px 12px;border-radius:10px;font-size:12px;font-weight:bold;line-height:1.7">لا توجد كلمة سر مسجلة لهذا الحساب — أدخل كلمة سر جديدة فقط لتسجيلها لأول مرة.</div>'
                : ''}
              ${noPassSet
                ? ''
                : `<div>
          ...
          onRender: (modalEl, closeModal) => {
            ...
            if (form) {
              form.onsubmit = (e) => {
                e.preventDefault();
                const current = noPassSet ? '' : modalEl.querySelector('#cp-current').value;
    ...
    // Logout Handlers
    const performLogout = () => {
      // V3.25: always leave sandbox mode on logout so the RAM state is restored
      // and the login screen never shows sandbox-mutated data.
      if (window.isSandboxActive && window.isSandboxActive()) window.exitSandboxMode();
      window.logout();
    ...
    // V3.18 — Quick sync actions dropdown (sync now / open sheet / direction).
    const syncMenuBtn = document.getElementById('sync-menu-btn');
    const syncMenuDropdown = document.getElementById('sync-menu-dropdown');
    const closeSyncMenu = () => {
      if (syncMenuDropdown) syncMenuDropdown.classList.add('hidden');
    };

    if (syncMenuBtn && syncMenuDropdown) {
      syncMenuBtn.onclick = (e) => {
        e.stopPropagation();
        const willOpen = syncMenuDropdown.classList.contains('hidden');
        syncMenuDropdown.classList.toggle('hidden');
        if (willOpen && typeof syncQuickFromConfig === 'function') syncQuickFromConfig();
      };
      document.addEventListener('click', (e) => {
        if (!syncMenuDropdown.contains(e.target) && !syncMenuBtn.contains(e.target)) {
          syncMenuDropdown.classList.add('hidden');
        }
      });
    }

    // V3.18 — Manual Google Sheets sync from the sync dropdown
    ...
    if (manualSyncBtn) {
      manualSyncBtn.onclick = async () => {
        ...
        try {
          ...
          window.showToast('تمت المزامنة مع Google Sheets بنجاح', 'success');
        } catch (err) {
          window.showToast('فشلت المزامنة: ' + (err && err.message ? err.message : String(err)), 'error');
        } finally {
          btn.disabled = false;
    ...
    // V3.18 — Open the linked Google Sheet from the sync dropdown
    ...
    // V3.18 — Quick sync-type selector (both / export / import / off).
    // Changes are saved to localStorage and mirrored to Firestore instantly.
    const quickDir = document.getElementById('sync-direction-quick');
    const DIR_LABELS = { both: 'بالاتجاهين', export: 'تصدير فقط', import: 'استيراد فقط' };
    const syncQuickFromConfig = () => {
      if (!quickDir) return;
      if (document.activeElement === quickDir) return; // don't clobber while user is picking
      const C = window.GoogleSheetsSync;
      if (!C || typeof C.getConfig !== 'function') return;
      const c = C.getConfig();
      if (!c) return;
      const val = c.enabled ? (['both', 'export', 'import'].indexOf(c.direction) >= 0 ? c.direction : 'export') : 'off';
      if (quickDir.value !== val) quickDir.value = val;
      const badge = document.getElementById('sync-dir-badge');
      if (badge) badge.textContent = val === 'off' ? 'متوقفة' : DIR_LABELS[val] || val;
    };
    if (quickDir) {
      quickDir.onchange = () => {
        if (!window.isAuthenticated()) return;
        const v = quickDir.value;
        if (window.GoogleSheetsSync && typeof window.GoogleSheetsSync.setQuickDirection === 'function') {
          window.GoogleSheetsSync.setQuickDirection(v);
          if (window.showToast) {
            window.showToast(v === 'off'
              ? '⏸ تم إيقاف المزامنة التلقائية'
              : '✓ تم تغيير نوع المزامنة إلى: ' + (DIR_LABELS[v] || v), 'success');
          }
        }
      };
      syncQuickFromConfig();
      window.addEventListener('bms-config-hydrated', syncQuickFromConfig);
      window.addEventListener('bms-data-synced', syncQuickFromConfig);
    }

    // V3.18 — "إعدادات الربط والسحابة 🔐" in the account dropdown
    const cloudSettingsBtn = document.getElementById('btn-open-cloud-settings');
    if (cloudSettingsBtn) {
      cloudSettingsBtn.onclick = () => {
        if (!window.isAdmin()) {
          if (window.showToast) window.showToast('هذه الإعدادات مخصصة للمدير فقط', 'error');
          return;
        }
        if (typeof window.openSyncCloudModal === 'function') {
          window.openSyncCloudModal();
        }
      };
    }
  }
  ...
  navigateTo(viewName) {
    ...
    if (role === 'storekeeper' && viewName !== 'products') {
      ...
      viewName = 'orders';
    } else if (role === 'employee' && (viewName === 'users' || viewName === 'reports' || viewName === 'suppliers' || viewName === 'payments')) {
