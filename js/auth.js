...
/**
 * Admin User Creation without session overwrite (Fixes Session Overwrite Bug)
 * Writes user record to Firestore users collection without switching active Admin Auth session
 */
window.createNewUserAccount = function({ name, email, password, role }) {
  ...
  // Add document to Firestore users collection WITHOUT altering Admin's active session!
