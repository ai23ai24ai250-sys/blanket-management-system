...
window.createPaymentRecord = function({ entityType, entityId, entityName, amount, date, paymentMethod = 'cash', notes = '', isDownPayment = false, createdBy = 'المدير العام', type = '', refOrderId = '', cycleKey = '' }) {
  const numericAmount = window.round2(parseFloat(amount));
  ...
  if (refOrderId && cycleKey) {
    ...
    if (existing) {
      ...
      const signedDelta = window.round2(numericAmount < 0 ? (numericAmount - oldAmount) : numericAmount);
      if (entityType === 'customer') {
        const customer = window.getCustomerById(entityId);
        if (customer) {
          window.updateCustomer(entityId, { paid: window.round2(Math.max(0, (Number(customer.paid) || 0) + signedDelta)) });
        }
      } else if (entityType === 'supplier') {
        const supplier = window.getSupplierById(entityId);
        if (supplier) {
          window.updateSupplier(entityId, { paid: window.round2(Math.max(0, (Number(supplier.paid) || 0) + signedDelta)) });
        }
      }
    ...
    if (entityType === 'customer') {
      ...
      if (customer) {
        ...
        if (numericAmount > maxRemaining) {
          throw new Error(`المبلغ المدخل أكبر من إجمالي المديونية المتبقية على العميل (${window.formatCurrency(maxRemaining)}) — يتجاوز إجمالي الرصيد المتبقي`);
        }
      }
    } else if (entityType === 'supplier') {
      const supplier = window.getSupplierById(entityId);
      if (supplier) {
        ...
        if (numericAmount > maxRemaining) {
          throw new Error(`المبلغ المدخل أكبر من إجمالي المديونية المستحقة للمورد (${window.formatCurrency(maxRemaining)}) — يتجاوز إجمالي الرصيد المستحق`);
        }
      }
    }
  }
  ...
  if (entityType === 'customer') {
    const customer = window.getCustomerById(entityId);
    if (customer) {
      const updatedPaid = window.round2((Number(customer.paid) || 0) + numericAmount);
      ...
      const updatedBalance = numericAmount < 0
        ...
        : (isDownPayment
            ? (Number(customer.remainingBalance) || 0)
            : window.round2(Math.max(0, (Number(customer.remainingBalance) || 0) - numericAmount)));
      ...
      if (!isDownPayment && numericAmount > 0) {
        ...
        for (const order of unpaidOrders) {
          ...
          const allocated = window.round2(Math.min(remainingToAllocate, orderRemaining));
          const newRemaining = window.round2(orderRemaining - allocated);
          const newDownPayment = window.round2((Number(order.downPayment) || 0) + allocated);
    ...
    if (supplier) {
      ...
      const updatedPaid = numericAmount < 0
        ...
        : window.round2((Number(supplier.paid) || 0) + numericAmount);
      const updatedBalance = numericAmount < 0
        ? (Number(supplier.remainingBalance) || 0)
        : window.round2(Math.max(0, (Number(supplier.remainingBalance) || 0) - numericAmount));
...
window.getTotalCustomerReceivables = function() {
  // V3.16 — Receivables are derived DIRECTLY from ACTIVE orders (new/delivered/
  // completed) via the order-level remaining formula, so cancelled/returned orders
  // (always settled, remaining 0) can never inflate the debt cards — regardless of
  // any stale customer.remainingBalance in legacy data.
  return window.round2(window.getOrders()
    .filter(o => window.isActiveOrderStatus(o.status))
    .reduce((sum, o) => sum + window.getOrderRemainingAmount(o), 0));
};

window.getTotalSupplierPayables = function() {
  const suppliers = window.getCollection(window.STORAGE_KEYS.SUPPLIERS);
  return window.round2(suppliers.reduce((sum, s) => sum + window.toNumber(s.remainingBalance), 0));
};

window.getTotalPaymentsCollected = function() {
  const payments = window.getPayments();
  return window.round2(payments.reduce((sum, p) => sum + window.toNumber(p.amount), 0));
};
