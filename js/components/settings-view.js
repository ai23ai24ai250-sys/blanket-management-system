...
window.renderSettingsView = function () {
  ...
  const generalCard = `
    <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-lg space-y-4">
      ...
      <div>
        <label class="block font-bold text-slate-300 text-xs mb-1.5">الوضع الليلي / النهاري</label>
        <select id="gen-theme" class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-brand-500">
          <option value="dark"${gs.theme !== 'light' ? ' selected' : ''}>🌙 الوضع الليلي</option>
          <option value="light"${gs.theme === 'light' ? ' selected' : ''}>☀️ الوضع النهاري</option>
        </select>
      </div>
...
window.setupSettingsEvents = function (container) {
  ...
  if (themeSel) {
    themeSel.addEventListener('change', () => {
      saveGeneral({ theme: themeSel.value });
      if (window.showToast) window.showToast(themeSel.value === 'light' ? '☀️ تم التبديل للوضع النهاري' : '🌙 تم التبديل للوضع الليلي', 'success');
    });
  }
