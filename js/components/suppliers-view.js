...
window.openAddSupplierModal = function(supplierToEdit = null, refreshParentFn = null) {
    ...
    <form id="form-supplier" class="space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="text" id="sup-city-manual" placeholder="اكتب اسم المدينة / المركز يدوياً..." class="hidden mt-2 w-full px-4 py-2.5 bg-slate-900 border border-amber-700/60 rounded-xl text-white font-bold transition-all">
        </div>
      </div>
  ...
  window.openModal({
    ...
    onRender: (modalEl, closeModal) => {
      ...
      const cityManualInput = modalEl.querySelector('#sup-city-manual');
...
window.openSupplierReturnModal = function(defaultSupplierId = '', refreshParentFn = null) {
  ...
  // V3.19 — Supplier-scoped product filtering: once a supplier is selected, the
  // product dropdowns only offer that supplier's products so a purchase return
  // can never accidentally pick another supplier's goods. With no supplier yet,
  // all products are offered as a fallback.
  let selectedSupplierId = defaultSupplierId || '';
  const matchesSupplier = (p) => {
    if (!selectedSupplierId) return true;
    return (p && (p.supplierId === selectedSupplierId || (p.supplierName && selectedSupplierId && p.supplierName === (suppliers.find(s => s.id === selectedSupplierId) || {}).name)));
  };
  const getFilteredProducts = () => products.filter(matchesSupplier);
  const firstFilteredProduct = () => getFilteredProducts()[0] || products[0] || null;

  let returnLineItems = [{
    id: Date.now(),
    productId: firstFilteredProduct()?.id || '',
    productName: firstFilteredProduct()?.name || '',
    quantity: 1,
    unitCost: firstFilteredProduct()?.purchasePrice || 0
  }];
    ...
    onRender: (modalEl, closeModal) => {
      ...
      const renderReturnRows = () => {
        ...
        rowsContainer.innerHTML = returnLineItems.map((item, idx) => {
          const pObj = filteredProducts.find(p => p.id === item.productId) || products.find(p => p.id === item.productId);
          return `
            <div class="return-row grid grid-cols-12 gap-3 p-3 bg-slate-800/60 rounded-xl border border-slate-700/80 items-center" data-row-index="${idx}">
              <div class="col-span-12 sm:col-span-5">
                <select class="return-product-select w-full px-2.5 py-2 bg-slate-900 border border-slate-700 rounded-lg text-xs text-white focus:outline-none focus:border-orange-500">
                  ...
                  ${filteredProducts.map(p => `<option value="${p.id}" ${p.id === item.productId ? 'selected' : ''}>${p.name} (المخزون: ${p.stock ?? 0})</option>`).join('')}
                </select>
              </div>
      ...
      modalEl.querySelector('#sret-add-product-row').onclick = () => {
        const first = firstFilteredProduct();
        returnLineItems.push({
          id: Date.now(),
          productId: first?.id || '',
          productName: first?.name || '',
          quantity: 1,
          unitCost: first?.purchasePrice || 0
        });
        renderReturnRows();
        attachRowListeners();
        updateTotal();
      ...
      // V3.19 — Switching the supplier re-scopes every product dropdown to that
      // supplier's products and re-seeds the first row from them.
      modalEl.querySelector('#sret-supplier-select').onchange = () => {
        selectedSupplierId = modalEl.querySelector('#sret-supplier-select').value;
        const first = firstFilteredProduct();
        returnLineItems = [{
          id: Date.now(),
          productId: first?.id || '',
      ...
      modalEl.querySelector('#form-supplier-return').onsubmit = (e) => {
        ...
        if (selectedSupplierId) {
          const foreign = items.find(i => {
            const prd = window.getProductById(i.productId);
            return prd && prd.supplierId && prd.supplierId !== selectedSupplierId;
          });
          if (foreign) {
            window.showToast('المنتج المحدد لا يخص هذا المورد — اختر منتجاً من قائمة المورد المحدد', 'error');
            return false;
          }
        }
