...
window.createOrder = function({ customerInfo, items, downPayment = 0, shippingCost = 0, shippingPayer = 'customer', extraExpenses = 0, extraExpensesPayer = 'customer', status = 'delivered', createdBy = 'المدير العام', directShipping = false, depositType = 'custom' }) {
  ...
  const remainingBalance = (status === 'cancelled' || status === 'returned') ? 0 : Math.max(0, totalAmount - dp);
...
window.applyOrderFulfillment = function(order) {
  ...
  const updateCustomerLedger = () => {
    ...
    const newBalance = (Number(customer.remainingBalance) || 0) + remaining;
...
window.updateOrderStatus = function(orderId, newStatus, refundAmount, reactivationDeposit) {
  ...
  if ((oldStatus === 'cancelled' || oldStatus === 'returned') && newStatus === 'new') {
    ...
    const retainedDefault = Math.max(0, (Number(currentOrder.downPayment) || 0) - (Number(currentOrder.refundedAmount) || 0));
    const reCreditAmount = (Number(reactivationDeposit) > 0) ? Number(reactivationDeposit) : retainedDefault;
    if (reCreditAmount > 0) {
      window.createPaymentRecord({
        entityType: 'customer',
