    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-4">
        ...
        <div class="p-4 bg-sky-950/30 rounded-xl border border-sky-800/40">
          ...
          <span data-shipping-revenue class="text-lg font-extrabold text-sky-400 num-font">${window.formatCurrency(calc.shippingRevenueIncome || 0)}</span>
...
function renderExpensesReport() {
  ...
  const op = window.getCurrentOperatingExpenses ? window.getCurrentOperatingExpenses() : null;
  const displayTotal = op ? op.total : total;
    ...
    <div class="space-y-6">
      <div class="flex items-center justify-between bg-slate-850 p-4 rounded-xl border border-slate-800">
        ...
        <div class="flex items-center gap-4">
          <div class="text-left">
            <span class="text-xs text-slate-400 block">إجمالي المصروفات المستحقة حالياً:</span>
            <span class="text-lg font-extrabold text-rose-400 num-font">${window.formatCurrency(displayTotal)}</span>
            ${op && op.recurringThisMonth > 0 ? `<span class="block text-[10px] font-bold text-purple-300">منها دورية مستحقة هذا الشهر: ${window.formatCurrency(op.recurringThisMonth)}</span>` : ''}
            ${op && op.recurringFuture > 0 ? `<span class="block text-[10px] font-bold text-slate-500">دورية لم يستحق موعدها بعد (قادمة): ${window.formatCurrency(op.recurringFuture)}</span>` : ''}
          </div>
            ...
            ` : expenses.map(e => `
              <tr>
                ...
                <td class="text-xs text-slate-400">
                  ${e.date}
                  ${e.recurring === true ? `<div class="mt-1"><span class="px-2 py-0.5 bg-purple-500/20 text-purple-300 border border-purple-500/30 rounded text-[10px] font-bold">شهري 🔁 استحقاق يوم ${e.dueDay}</span></div>` : ''}
                </td>
                <td class="text-xs text-slate-400">${e.notes || '—'}</td>
                <td>
                  <div class="flex items-center gap-1">
                    <button class="btn-edit-expense text-sky-400 hover:text-sky-300 p-1 hover:bg-sky-950/40 rounded-lg" data-expense-id="${e.id}">
...
window.setupReportsEvents = function(container) {
  ...
  if (tabExpenses) {
    tabExpenses.onclick = () => {
      ...
      bodyEl.querySelectorAll('.btn-edit-expense').forEach(b => {
        b.onclick = () => {
          const eId = b.getAttribute('data-expense-id');
          window.openEditExpenseModal(eId, () => {
            bodyEl.innerHTML = renderExpensesReport();
            if (window.lucide) window.lucide.createIcons({ props: {}, nameAttr: 'data-lucide' });
          });
        };
...
window.openAddExpenseModal = function(refreshParentFn = null) {
  const contentHTML = `
    <form id="form-add-expense" class="space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-3 bg-slate-950/40 rounded-xl border border-slate-800">
        <label class="flex items-center gap-2.5 cursor-pointer select-none">
          <input type="checkbox" id="exp-recurring" class="w-4 h-4 accent-purple-500 shrink-0">
          <span class="text-xs font-bold text-purple-300">مصروف شهري متكرر 🔁 (يُخصم من الصافي تلقائياً كل شهر)</span>
        </label>
        <div id="exp-due-wrap" class="hidden">
          <label class="block text-xs font-bold text-slate-300 mb-1.5">يوم الاستحقاق الشهري (1 - 31)</label>
          <input type="number" id="exp-due-day" min="1" max="31" placeholder="مثال: 5" class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-white num-font">
        </div>
      </div>
  ...
  window.openModal({
    ...
    onRender: (modalEl, closeModal) => {
      const recurringCheck = modalEl.querySelector('#exp-recurring');
      const dueWrap = modalEl.querySelector('#exp-due-wrap');
      if (recurringCheck && dueWrap) {
        recurringCheck.addEventListener('change', () => {
          dueWrap.classList.toggle('hidden', !recurringCheck.checked);
        });
      }
      modalEl.querySelector('#form-add-expense').onsubmit = (e) => {
        ...
        try {
          ...
          window.createExpense({
            ...
            recurring,
            dueDay: recurring ? modalEl.querySelector('#exp-due-day').value : null
          });

          window.showToast(recurring ? 'تم قيد المصروف الشهري — يُخصم من الأرباح في موعد استحقاقه' : 'تم قيد المصروف بنجاح وخصم قيمته من الأرباح', 'success');
          closeModal();
...
window.openEditExpenseModal = function(expenseId, refreshParentFn = null) {
  const expense = (window.getExpenses ? window.getExpenses() : []).find(e => e.id === expenseId);
  if (!expense) {
  ...
  const contentHTML = `
    <form id="form-edit-expense" class="space-y-4">
      <div>
        <label class="block text-xs font-bold text-slate-300 mb-1.5">بيان المصروف *</label>
        <input type="text" id="exp-title" required value="${expense.title}" placeholder="مثال: فاتورة كهرباء المحل / إيجار شهر يوليو" class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-white">
      </div>

      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label class="block text-xs font-bold text-slate-300 mb-1.5">قيمة المصروف (ج.م) *</label>
          <input type="number" id="exp-amount" min="1" required value="${expense.amount}" placeholder="0" class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-rose-400 font-extrabold text-lg num-font">
        </div>

        <div>
          <label class="block text-xs font-bold text-slate-300 mb-1.5">فئة المصروف</label>
          <select id="exp-category" class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-white font-bold">
            <option value="عمومية" ${expense.category === 'عمومية' ? 'selected' : ''}>مصروفات عمومية</option>
            <option value="إيجارات" ${expense.category === 'إيجارات' ? 'selected' : ''}>إيجار المحل والمخزن</option>
            <option value="كهرباء ومرافق" ${expense.category === 'كهرباء ومرافق' ? 'selected' : ''}>كهرباء ومياه ومرافق</option>
            <option value="أجور ومرتبات" ${expense.category === 'أجور ومرتبات' ? 'selected' : ''}>أجور ومرتبات موظفين</option>
            <option value="تغليف ومطبوعات" ${expense.category === 'تغليف ومطبوعات' ? 'selected' : ''}>أكياس وتغليف ومطبوعات</option>
            <option value="شحن ونقل" ${expense.category === 'شحن ونقل' ? 'selected' : ''}>شحن ونقل بضائع</option>
          </select>
        </div>
      </div>
      ...
      <div>
        <label class="block text-xs font-bold text-slate-300 mb-1.5">التاريخ</label>
        <input type="date" id="exp-date" value="${expense.date}" class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-white num-font">
      </div>

      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-3 bg-slate-950/40 rounded-xl border border-slate-800">
        <label class="flex items-center gap-2.5 cursor-pointer select-none">
          <input type="checkbox" id="exp-recurring" class="w-4 h-4 accent-purple-500 shrink-0" ${isRecurring ? 'checked' : ''}>
          <span class="text-xs font-bold text-purple-300">مصروف شهري متكرر 🔁 (يُخصم من الصافي تلقائياً كل شهر)</span>
        </label>
        <div id="exp-due-wrap" class="${isRecurring ? '' : 'hidden'}">
          <label class="block text-xs font-bold text-slate-300 mb-1.5">يوم الاستحقاق الشهري (1 - 31)</label>
          <input type="number" id="exp-due-day" min="1" max="31" value="${isRecurring ? expense.dueDay : ''}" placeholder="مثال: 5" class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-white num-font">
        </div>
      </div>

      <div>
        <label class="block text-xs font-bold text-slate-300 mb-1.5">ملاحظات إضافية</label>
        <input type="text" id="exp-notes" value="${expense.notes || ''}" placeholder="ملاحظات توضيحية..." class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-white">
      </div>

      <div class="flex justify-end gap-3 pt-2">
        <button type="submit" class="px-6 py-2.5 bg-sky-600 hover:bg-sky-500 text-white font-bold text-sm rounded-xl shadow-lg shadow-sky-600/20">
          حفظ التعديلات
        </button>
      </div>
    </form>
  ...
  window.openModal({
    ...
    onRender: (modalEl, closeModal) => {
      ...
      modalEl.querySelector('#form-edit-expense').onsubmit = (e) => {
        e.preventDefault();
        try {
          const recurring = recurringCheck ? recurringCheck.checked : false;
          window.updateExpense(expenseId, {
            title: modalEl.querySelector('#exp-title').value,
            amount: modalEl.querySelector('#exp-amount').value,
            category: modalEl.querySelector('#exp-category').value,
            date: modalEl.querySelector('#exp-date').value,
          ...
          window.showToast('تم تعديل المصروف بنجاح', 'success');
          closeModal();
          if (refreshParentFn) refreshParentFn();
          else if (window.appInstance) window.appInstance.navigateTo('reports');
        } catch (err) {
