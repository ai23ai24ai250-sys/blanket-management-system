...
async function _reconcileStrandedLocalDocs(snapshot) {
  ...
  const src = snapshot || _snapshotLocalMirrors();
  const tombSet = {};
  window.getTombstones().forEach(id => { tombSet[id] = true; });
  const queuedIds = {};
  window.pendingOpsQueue().forEach(op => {
...
window.forcePushPendingToCloud = async function (snapshot, opts) {
  ...
  try {
    ...
    if (window.showToast && !opts.silent) window.showToast('⚠️ تعذر رفع السجلات المعلقة الآن — سيُعاد الإرسال تلقائياً', 'error');
    return 0;
  }
};
...
function _doStartSync() {
  ...
  const localSnapshot = _snapshotLocalMirrors();
...
window.initDB = function() {
  ...
  // are settled invoices (متبقي 0), and customer balances are recomputed from
  // ACTIVE orders + payments so no legacy cancelled/returned order can keep
  // inflating "المتبقي على العميل" or the debt cards.
...
/**
 * V3.16.1 — DEEP DATA RESET (Cloud + Local).
...
window.forceWipeDatabase = async function(providedAdminPassword = '') {
  ...
  if (window.showToast) window.showToast('تم مسح وتصفير القواعد السحابية والمحلية بنجاح — سيتم إعادة التحميل تلقائياً 🧹', 'success');
