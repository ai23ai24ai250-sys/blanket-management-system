...
window.createSupplierReturn = function({ supplierId, supplierName, items, refundType = 'debt', notes = '', createdBy = 'المدير العام' }) {
  ...
  const totalValue = processedItems.reduce((s, i) => s + i.subtotal, 0);
  ...
  const newPurchases = Math.max(0, oldPurchases - totalValue);
  const newBalance = Math.max(0, oldBalance - totalValue);
  const newPaid = Math.max(0, newPurchases - newBalance);
...
window.getTotalSupplierReturnsValue = function() {
  return window.getSupplierReturns().reduce((sum, r) => sum + (Number(r.totalValue) || 0), 0);
};
